...
def trainingFactor(initialValue, adjustingSize, trainingTimes, numOfValues):
	...
	for j in range(0, numOfValues):
		...
		for k in range(0, trainingTimes):
			...
			config.register_player(name="rate", algorithm=RatePlayer())
			game_result = start_poker(config, verbose=1)
			print game_result
			totalResult += game_result['players'][0]['stack'] 
			detailResult += "Round: " + str(k) + " result: " + str(game_result['players'][0]['stack'])
		...
		import cPickle
		result = open(detailResult + 'trainingResult' + ' foldThreshold' + str(RaisedPlayer.foldThreshold)+ '.pkl', 'w')
